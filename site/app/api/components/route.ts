...
import { COMPONENT_COLUMNS, auditStatement, isConstraintError } from '@/lib/repository';
...
export async function GET(request: Request) {
  try {
    ...
    const conditions = ['c.project_id = ?', 'c.disabled_at IS NULL'];
    const bindings: unknown[] = [projectId];
    if (status && allowedStatuses.has(status)) { conditions.push('c.current_status = ?'); bindings.push(status); }
    if (query) { conditions.push('(c.code LIKE ? OR c.name LIKE ? OR c.batch LIKE ? OR c.area LIKE ?)'); const like = `%${query}%`; bindings.push(like, like, like, like); }
    bindings.push(limit);
    const result = await database().prepare(`SELECT ${COMPONENT_COLUMNS} FROM components c WHERE ${conditions.join(' AND ')} ORDER BY c.updated_at DESC LIMIT ?`).bind(...bindings).all();
  ...
  try {
    ...
    try {
      await database().batch([
        database().prepare(`INSERT INTO components (id, project_id, code, name, type, specification, material, batch, building, floor, area, current_status, version, qr_token, notes, created_at, updated_at, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'UNRECORDED', 1, ?, ?, ?, ?, ?)`).bind(id, projectId, code, name, type, record.specification, record.material, record.batch, record.building, record.floor, record.area, record.qrToken, record.notes, now, now, actor.displayName),
        auditStatement(request, actor, 'COMPONENT_CREATED', 'component', id, projectId, null, record),
      ]);
    } catch (error) {

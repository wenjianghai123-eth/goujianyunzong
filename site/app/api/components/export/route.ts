...
export async function GET(request: Request) {
  try {
    ...
    const rows = await database().prepare(`SELECT code, name, type, specification, material, batch, building, floor, area, current_status AS status, updated_at AS updatedAt, updated_by AS updatedBy FROM components WHERE project_id = ? AND disabled_at IS NULL ORDER BY code`).bind(projectId).all<Record<string, unknown>>();
    ...
    const csv = `\uFEFF${headers.map(csvCell).join(',')}\r\n${rows.results.map((row) => keys.map((key) => csvCell(row[key])).join(',')).join('\r\n')}`;

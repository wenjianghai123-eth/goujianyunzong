...
export async function GET(request: Request, context: Context) {
  try {
    ...
    const component = await database().prepare(`SELECT project_id AS projectId, code, name, qr_token AS qrToken FROM components WHERE id = ? AND disabled_at IS NULL`).bind(id).first<{ projectId: string; code: string; name: string; qrToken: string }>();
...
export async function POST(request: Request, context: Context) {
  try {
    const actor = await requireActor();
    const { id } = await context.params;
    const component = await database().prepare(`SELECT project_id AS projectId, qr_token AS oldToken FROM components WHERE id = ? AND disabled_at IS NULL`).bind(id).first<{ projectId: string; oldToken: string }>();
    if (!component) throw new HttpError(404, '未找到该构件');
    await requireProjectAccess(actor, component.projectId, 'admin');
    const qrToken = makeQrToken();
    await database().batch([
      database().prepare(`UPDATE components SET qr_token = ?, version = version + 1, updated_at = ? WHERE id = ?`).bind(qrToken, new Date().toISOString(), id),
      auditStatement(request, actor, 'QR_TOKEN_REGENERATED', 'component', id, component.projectId, { tokenRevoked: true }, { tokenRegenerated: true }),
    ]);

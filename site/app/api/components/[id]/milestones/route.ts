...
export async function POST(request: Request, context: Context) {
  try {
    ...
    await database().batch([
      database().prepare(`INSERT INTO milestones (id, component_id, type, actual_at, operator_id, operator_name, vehicle_no, receiver, location, remark, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, componentId, type, record.actualAt, actor.userId, actor.displayName, record.vehicleNo, record.receiver, record.location, record.remark, record.createdAt),
      auditStatement(request, actor, 'MILESTONE_CREATED', 'component', componentId, component.projectId, null, record),
    ]);
